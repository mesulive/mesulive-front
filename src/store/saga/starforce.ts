...
import {getReachableStar, Option, StarforceSimState, starforceSim} from "lib/starforce";
...
function* calc() {
  ...
  const option: Option = {
    ...
    detail: {
      simNum: simNum!,
      safeguardArr: safeguardArr,
      starcatchArr: starcatchArr,
      event,
      discount
    }
  };
  ...
  do {
    try {
      ...
      for (let res of result.resultArr) {
...
function* handleErrorMessage() {
  ...
  let message: { [index: string]: string } = {};
